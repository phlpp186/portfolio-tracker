...
const CLIENT_ID   = "380889468418-3snhjp21aun8b8qp52mg5n5hl24dv7b7.apps.googleusercontent.com";
const SCOPES      = "https://www.googleapis.com/auth/spreadsheets";
const BASE        = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}`;

const TABS = {
  ASSETS: "assets", P2P_LOANS: "p2p_loans", P2P_INTEREST: "p2p_interest",
  RECURRING: "recurring_rules", REAL_ESTATE: "real_estate",
  COMMODITIES: "commodities", DIVIDENDS: "dividends", SNAPSHOTS: "snapshots",
};
...
function getToken() {
  return new Promise((resolve, reject) => {
    ...
    tokenClient.callback = (resp) => {
      ...
      // Token expires in 1 hour — clear it so next call re-auths
...
async function sheetsClear(tab) {
  const token = await getToken();
  const res = await fetch(`${BASE}/values/${encodeURIComponent(tab)}:clear`, {
...
async function sheetsAppend(tab, rows) {
  const token = await getToken();
  const res = await fetch(`${BASE}/values/${encodeURIComponent(tab)}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`, {
    method: "POST",
...
async function sheetsWrite(tab, rows) {
  ...
  if (rows.length > 0) await sheetsAppend(tab, rows);
}
...
async function fetchMetalPrice(name) {
  try {
    ...
    return usdPrice * (rateJson?.rates?.EUR || 0.92);
  } catch { return null; }
}
...
function formatPct(val) { return `${val>=0?"+":""}${(val||0).toFixed(2)}%`; }
...
function EmptyState({children}){return <div style={{background:"#161918",border:"1px solid #2A2D2B",borderRadius:"4px",padding:"60px",textAlign:"center",color:"#3A3D3B",fontSize:"13px"}}>{children}</div>;}
...
function SignInScreen({onSignIn,error}){
  return(
    <div style={{fontFamily:"Georgia, serif",background:"#0D0F0E",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center",maxWidth:"400px",padding:"48px"}}>
        <div style={{fontSize:"11px",letterSpacing:"4px",color:"#7A8077",textTransform:"uppercase",marginBottom:"16px"}}>Investment Portfolio</div>
        <div style={{fontSize:"32px",fontWeight:"400",color:"#E8E0D0",marginBottom:"8px"}}>Performance Tracker</div>
        <div style={{fontSize:"13px",color:"#5A6057",marginBottom:"48px"}}>Connected to Google Sheets</div>
        <button onClick={onSignIn} style={{background:"#C8A97E",border:"none",borderRadius:"2px",color:"#0D0F0E",fontSize:"12px",letterSpacing:"3px",padding:"16px 32px",cursor:"pointer",textTransform:"uppercase",fontWeight:"700",fontFamily:"Georgia, serif",width:"100%"}}>
          Sign in with Google
        </button>
        {error && <div style={{marginTop:"16px",fontSize:"12px",color:"#C87E7E"}}>{error}</div>}
        <div style={{marginTop:"24px",fontSize:"11px",color:"#3A4038",lineHeight:"1.6"}}>
          Your data is stored in your own Google Sheet.<br/>This app only accesses that one spreadsheet.
        </div>
      </div>
    </div>
  );
...
export default function PortfolioTracker() {
  const [authState, setAuthState] = useState("idle"); // idle | signing-in | ready
  const [authError, setAuthError] = useState(null);
  const [data, setData] = useState({ assets:[], p2pLoans:[], p2pInterest:[], recurringRules:[], realEstate:[], commodities:[], dividends:[], snapshots:[] });
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      const [aR,lR,iR,rR,pR,cR,dR,sR] = await Promise.all([
      ...
      const loaded = {
        assets: aR.map(rowToAsset), p2pLoans: lR.map(rowToLoan),
        p2pInterest: iR.map(rowToInt), recurringRules: rR.map(rowToRule),
        realEstate: pR.map(rowToProp), commodities: cR.map(rowToComm),
        dividends: dR.map(rowToDiv), snapshots: sR.map(rowToSnap),
      };

      // Apply recurring rules
      const todayStr = today();
      let newInterest = [...loaded.p2pInterest];
      let rulesChanged = false, autoAdded = 0;
      ...
      if (rulesChanged) {
        ...
        await sheetsWrite(TABS.P2P_INTEREST, newInterest.map(intToRow));
      }

      setStatus({type:"prices",msg:"Fetching live prices…"});
      await refreshPrices(loaded);

      setStatus({type:"success",msg: autoAdded > 0 ? `Loaded · ${autoAdded} interest entries auto-added` : "All data loaded"});
  ...
  // ── Refresh live prices ────────────────────────────────────────────────
  const refreshPrices = async (src) => {
    const current = src || data;
    const updatedAssets = [...current.assets];
    const updatedCommodities = [...current.commodities];
    const updates = {};

    await Promise.all([
      ...updatedAssets.map(async (a, idx) => {
        const price = a.type==="Crypto" ? await fetchCryptoPrice(a.ticker) : await fetchStockPrice(a.ticker);
    ...
    if (Object.keys(updates).length > 0) {
      ...
      const newData = {...(src||data), assets:updatedAssets, commodities:updatedCommodities};
      setData(newData);
  ...
  const updateData = useCallback(async (patch, tab, serializer) => {
    ...
    try {
      await sheetsWrite(tab, newData[key].map(serializer));
  ...
  const totalValue            = totalAssetsValue+totalP2PValue+totalRealEstateValue+totalCommoditiesValue;
  ...
  const handleAddAsset = async () => {
    ...
    await updateData({assets:[...data.assets,{id:String(Date.now()),name:assetForm.name,ticker:assetForm.ticker.toUpperCase(),type:assetForm.type,shares:Number(assetForm.shares),avgCost:Number(assetForm.avgCost)||Number(assetForm.currentPrice),currentPrice:Number(assetForm.currentPrice),lastUpdated:today()}]},TABS.ASSETS,assetToRow);
    setAssetForm({name:"",ticker:"",type:"Stock",shares:"",avgCost:"",currentPrice:""});setShowAddAsset(false);
  };
  ...
  const handleAddSnapshot = async () => {
    ...
    await updateData({snapshots:[...data.snapshots.filter(s=>s.date!==snapshotDate),snap]},TABS.SNAPSHOTS,snapToRow);
    setShowSnapshot(false);
  };
  const handleAddDividend = async () => { if(!dividendForm.amount||!dividendForm.date)return; await updateData({dividends:[...data.dividends,{id:String(Date.now()),...dividendForm,amount:Number(dividendForm.amount)}]},TABS.DIVIDENDS,divToRow);setDividendForm({assetId:"",amount:"",date:today(),note:""});setShowAddDividend(false); };
  const handleAddP2P = async () => { if(!p2pForm.platform||!p2pForm.amount||!p2pForm.interestRate)return; await updateData({p2pLoans:[...data.p2pLoans,{id:String(Date.now()),...p2pForm,amount:Number(p2pForm.amount),interestRate:Number(p2pForm.interestRate)}]},TABS.P2P_LOANS,loanToRow);setP2pForm({platform:"",loanId:"",amount:"",interestRate:"",startDate:today(),endDate:"",note:""});setShowAddP2P(false); };
  ...
  const handleAddCommodity = async () => { if(!commodityForm.name||!commodityForm.weight||!commodityForm.pricePerUnit)return; await updateData({commodities:[...data.commodities,{id:String(Date.now()),...commodityForm,weight:Number(commodityForm.weight),pricePerUnit:Number(commodityForm.pricePerUnit),lastUpdated:today()}]},TABS.COMMODITIES,commToRow);setCommodityForm({name:"Gold",weight:"",unit:"oz",pricePerUnit:"",note:""});setShowAddCommodity(false); };
  ...
  // ── Sign in gate ───────────────────────────────────────────────────────
  if (authState !== "ready") {
    return <SignInScreen onSignIn={handleSignIn} error={authError} loading={authState==="signing-in"} />;
  }

  const tabs = ["overview","holdings","real assets","p2p","recurring","history","income"];

  const statusColors = {loading:"#7A8077",saving:"#C8A97E",success:"#7EC89B",error:"#C87E7E",prices:"#7EB5C8"};

  return (
    <div style={{fontFamily:"Georgia, serif",background:"#0D0F0E",minHeight:"100vh",color:"#E8E0D0"}}>
      ...
      <div style={{borderBottom:"1px solid #2A2D2B",padding:"24px 32px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{textAlign:"right"}}>
          ...
          <div style={{fontSize:"12px",color:"#7A8077",marginTop:"2px"}}>{formatCurrency(totalAssetsValue)} sec · {formatCurrency(totalP2PValue)} P2P · {formatCurrency(totalRealEstateValue+totalCommoditiesValue)} real</div>
      ...
      {status && (
        <div style={{background:"#161918",borderBottom:"1px solid #2A2D2B",padding:"8px 32px",display:"flex",alignItems:"center",gap:"8px"}}>
          {(status.type==="loading"||status.type==="prices") && <span style={{width:"6px",height:"6px",borderRadius:"50%",background:statusColors[status.type],display:"inline-block",animation:"pulse 1s infinite"}}/>}
          <span style={{fontSize:"12px",color:statusColors[status.type]||"#7A8077"}}>{status.msg}</span>
        </div>
      )}
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px",marginBottom:"24px"}}>
              ...
              <div style={{background:"#161918",border:"1px solid #2A2D2B",borderRadius:"4px",padding:"20px 24px"}}>
                ...
                <div style={{fontSize:"11px",color:"#4A7A5A",marginTop:"4px"}}>{formatCurrency(totalDividends)} div · {formatCurrency(totalP2PInterest)} P2P</div>
              </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"24px"}}>
              ...
              <div style={{background:"#161918",border:"1px solid #2A2D2B",borderRadius:"4px",padding:"24px"}}>
                ...
                {snapshotChartData.length<2?<div style={{color:"#3A3D3B",textAlign:"center",padding:"40px 0",fontSize:"13px"}}>Save snapshots to track growth</div>:(
                  <ResponsiveContainer width="100%" height={160}>
                    <AreaChart data={snapshotChartData}>
                      <defs><linearGradient id="gT" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#C8A97E" stopOpacity={0.3}/><stop offset="95%" stopColor="#C8A97E" stopOpacity={0}/></linearGradient></defs>
                      ...
                      <Area type="monotone" dataKey="total" stroke="#C8A97E" strokeWidth={2} fill="url(#gT)" name="Total"/>
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </div>
                ...
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  ...
                  <tbody>
                    {data.assets.map((a,i)=>{
                      ...
                      return(
                        <tr key={a.id} style={{borderBottom:i<data.assets.length-1?"1px solid #1E2120":"none"}} onMouseEnter={e=>e.currentTarget.style.background="#1A1D1B"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                          <td style={{padding:"14px 16px"}}><div style={{fontSize:"14px",color:"#E8E0D0"}}>{a.ticker||a.name}</div><div style={{fontSize:"11px",color:"#5A6057"}}>{a.name}</div></td>
                          ...
                          <td style={{padding:"14px 16px"}}><div style={{color:"#E8E0D0",fontSize:"13px"}}>{formatCurrency(a.currentPrice)}</div>{priceUpdates[a.id]&&<div style={{fontSize:"9px",color:"#7EB5C8",letterSpacing:"1px"}}>LIVE</div>}</td>
                          <td style={{padding:"14px 16px",color:"#E8E0D0",fontSize:"14px",fontWeight:"500"}}>{formatCurrency(mv)}</td>
                          <td style={{padding:"14px 16px"}}><div style={{color:gain>=0?"#7EC89B":"#C87E7E",fontSize:"13px"}}>{formatCurrency(gain)}</div><div style={{color:gain>=0?"#4A8A6A":"#8A4A4A",fontSize:"11px"}}>{formatPct(gainPct)}</div></td>
        ...
        {activeTab==="real assets"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}>
              <div><div style={{fontSize:"10px",letterSpacing:"3px",color:"#5A6057",textTransform:"uppercase",marginBottom:"4px"}}>Real Estate</div><div style={{fontSize:"20px",fontWeight:"300",color:"#7EB5C8"}}>{formatCurrency(totalRealEstateValue)}</div></div>
                ...
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  ...
                  <tbody>
                    {data.realEstate.map((p,i)=>(
                      <tr key={p.id} style={{borderBottom:i<data.realEstate.length-1?"1px solid #1E2120":"none"}} onMouseEnter={e=>e.currentTarget.style.background="#1A1D1B"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}>
              <div><div style={{fontSize:"10px",letterSpacing:"3px",color:"#5A6057",textTransform:"uppercase",marginBottom:"4px"}}>Commodities</div><div style={{fontSize:"20px",fontWeight:"300",color:"#C8BC7E"}}>{formatCurrency(totalCommoditiesValue)}</div></div>
              <button onClick={()=>setShowAddCommodity(true)} style={primaryBtnStyle}>+ Add Commodity</button>
            </div>
            {data.commodities.length===0?<EmptyState>No commodities yet.</EmptyState>:(
              <div style={{background:"#161918",border:"1px solid #2A2D2B",borderRadius:"4px",overflow:"hidden"}}>
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  ...
                  <tbody>
                    {data.commodities.map((c,i)=>{
                      ...
                      return(
                        <tr key={c.id} style={{borderBottom:i<data.commodities.length-1?"1px solid #1E2120":"none"}} onMouseEnter={e=>e.currentTarget.style.background="#1A1D1B"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                          <td style={{padding:"14px 16px",color:"#E8E0D0",fontSize:"14px"}}>{c.name}</td>
                          <td style={tdStyle}>{c.weight} {c.unit}</td>
                          <td style={{padding:"14px 16px"}}><div style={tdStyle}>{formatCurrency(c.pricePerUnit)}/{c.unit}</div>{priceUpdates[`comm-${c.id}`]&&<div style={{fontSize:"9px",color:"#7EB5C8",letterSpacing:"1px"}}>LIVE</div>}</td>
                ...
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  ...
                  <tbody>
                    {data.p2pLoans.map((l,i)=>{
                      ...
                      return(
                        <tr key={l.id} style={{borderBottom:i<data.p2pLoans.length-1?"1px solid #1E2120":"none"}} onMouseEnter={e=>e.currentTarget.style.background="#1A1D1B"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
        ...
        {activeTab==="recurring"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
              <div><div style={{fontSize:"10px",letterSpacing:"3px",color:"#5A6057",textTransform:"uppercase",marginBottom:"4px"}}>Recurring Daily Interest</div><div style={{fontSize:"13px",color:"#7A8077"}}>Missed days backfill automatically on each open.</div></div>
            ...
            {snapshotChartData.length>=2&&(
              <div style={{background:"#161918",border:"1px solid #2A2D2B",borderRadius:"4px",padding:"24px",marginBottom:"24px"}}>
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <AreaChart data={snapshotChartData}>
                    <defs>
                      <linearGradient id="gS" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#C8A97E" stopOpacity={0.4}/><stop offset="95%" stopColor="#C8A97E" stopOpacity={0}/></linearGradient>
                      <linearGradient id="gP" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#7EC89B" stopOpacity={0.4}/><stop offset="95%" stopColor="#7EC89B" stopOpacity={0}/></linearGradient>
                      <linearGradient id="gR" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#7EB5C8" stopOpacity={0.4}/><stop offset="95%" stopColor="#7EB5C8" stopOpacity={0}/></linearGradient>
                      <linearGradient id="gC" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#C8BC7E" stopOpacity={0.4}/><stop offset="95%" stopColor="#C8BC7E" stopOpacity={0}/></linearGradient>
                    </defs>
                    ...
                    <Area type="monotone" dataKey="securities" stroke="#C8A97E" strokeWidth={2} fill="url(#gS)" name="Securities" stackId="1"/>
                    <Area type="monotone" dataKey="p2p" stroke="#7EC89B" strokeWidth={2} fill="url(#gP)" name="P2P Credit" stackId="1"/>
                    <Area type="monotone" dataKey="realEstate" stroke="#7EB5C8" strokeWidth={2} fill="url(#gR)" name="Real Estate" stackId="1"/>
                    <Area type="monotone" dataKey="commodities" stroke="#C8BC7E" strokeWidth={2} fill="url(#gC)" name="Commodities" stackId="1"/>
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <Modal title={editingAsset?"Edit Holding":"Add Holding"} onClose={()=>{setShowAddAsset(false);setEditingAsset(null);}}>
          {["name","ticker","shares","avgCost","currentPrice"].map(f=>(
            <div key={f} style={{marginBottom:"16px"}}>
              ...
              <input type={["shares","avgCost","currentPrice"].includes(f)?"number":"text"} style={inputStyle} placeholder={f==="ticker"?"e.g. VWCE.AS, BTC, AAPL":f==="avgCost"?"defaults to current price":""}
                value={editingAsset?editingAsset[f]:assetForm[f]} onChange={e=>editingAsset?setEditingAsset({...editingAsset,[f]:e.target.value}):setAssetForm({...assetForm,[f]:e.target.value})}/>
            </div>
          ))}
      ...
      {(showAddProperty||editingProperty)&&(
        <Modal title={editingProperty?"Edit Property":"Add Property"} onClose={()=>{setShowAddProperty(false);setEditingProperty(null);}}>
          {[{f:"name",l:"Property Name / Address",t:"text"},{f:"currentValue",l:"Current Estimated Value (€)",t:"number"},{f:"note",l:"Note (optional)",t:"text"}].map(({f,l,t})=>(
            <div key={f} style={{marginBottom:"16px"}}><label style={labelStyle}>{l}</label><input type={t} style={inputStyle} value={editingProperty?(editingProperty[f]||""):propertyForm[f]} onChange={e=>editingProperty?setEditingProperty({...editingProperty,[f]:e.target.value}):setPropertyForm({...propertyForm,[f]:e.target.value})}/></div>
          ))}
      ...
      {(showAddCommodity||editingCommodity)&&(
        <Modal title={editingCommodity?"Edit Commodity":"Add Commodity"} onClose={()=>{setShowAddCommodity(false);setEditingCommodity(null);}}>
          <div style={{marginBottom:"16px"}}><label style={labelStyle}>Asset Name</label><input type="text" style={inputStyle} placeholder="Gold, Silver, Platinum…" value={editingCommodity?editingCommodity.name:commodityForm.name} onChange={e=>editingCommodity?setEditingCommodity({...editingCommodity,name:e.target.value}):setCommodityForm({...commodityForm,name:e.target.value})}/></div>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:"12px",marginBottom:"16px"}}>
            <div><label style={labelStyle}>Weight</label><input type="number" style={inputStyle} value={editingCommodity?editingCommodity.weight:commodityForm.weight} onChange={e=>editingCommodity?setEditingCommodity({...editingCommodity,weight:e.target.value}):setCommodityForm({...commodityForm,weight:e.target.value})}/></div>
            <div><label style={labelStyle}>Unit</label><select style={{...inputStyle,cursor:"pointer"}} value={editingCommodity?editingCommodity.unit:commodityForm.unit} onChange={e=>editingCommodity?setEditingCommodity({...editingCommodity,unit:e.target.value}):setCommodityForm({...commodityForm,unit:e.target.value})}>{COMMODITY_UNITS.map(u=><option key={u}>{u}</option>)}</select></div>
          </div>
          <div style={{marginBottom:"16px"}}><label style={labelStyle}>Price per {editingCommodity?editingCommodity.unit:commodityForm.unit} (€)</label><input type="number" style={inputStyle} value={editingCommodity?editingCommodity.pricePerUnit:commodityForm.pricePerUnit} onChange={e=>editingCommodity?setEditingCommodity({...editingCommodity,pricePerUnit:e.target.value}):setCommodityForm({...commodityForm,pricePerUnit:e.target.value})}/></div>
          ...
          <div style={{marginBottom:"20px"}}><label style={labelStyle}>Note (optional)</label><input type="text" style={inputStyle} value={editingCommodity?(editingCommodity.note||""):commodityForm.note} onChange={e=>editingCommodity?setEditingCommodity({...editingCommodity,note:e.target.value}):setCommodityForm({...commodityForm,note:e.target.value})}/></div>
      ...
      {(showAddP2P||editingP2P)&&(
        <Modal title={editingP2P?"Edit P2P Loan":"Add P2P Loan"} onClose={()=>{setShowAddP2P(false);setEditingP2P(null);}}>
          {[{f:"platform",l:"Platform / Lender",t:"text"},{f:"loanId",l:"Loan ID (optional)",t:"text"},{f:"amount",l:"Invested Amount (€)",t:"number"},{f:"interestRate",l:"Annual Interest Rate (%)",t:"number"},{f:"startDate",l:"Start Date",t:"date"},{f:"endDate",l:"End Date (optional)",t:"date"},{f:"note",l:"Note (optional)",t:"text"}].map(({f,l,t})=>(
            <div key={f} style={{marginBottom:"16px"}}><label style={labelStyle}>{l}</label><input type={t} style={inputStyle} value={editingP2P?(editingP2P[f]||""):p2pForm[f]} onChange={e=>editingP2P?setEditingP2P({...editingP2P,[f]:e.target.value}):setP2pForm({...p2pForm,[f]:e.target.value})}/></div>
          ))}
      ...
      {showAddInterest&&(
        <Modal title="Log Interest Payment" onClose={()=>setShowAddInterest(false)}>
          <div style={{marginBottom:"16px"}}><label style={labelStyle}>Loan / Platform</label><select style={{...inputStyle,cursor:"pointer"}} value={interestForm.loanId} onChange={e=>setInterestForm({...interestForm,loanId:e.target.value})}><option value="">— Select loan (optional)</option>{data.p2pLoans.map(l=><option key={l.id} value={l.id}>{l.platform}{l.loanId?` · ${l.loanId}`:""}</option>)}</select></div>
          {[{f:"amount",l:"Amount (€)",t:"number"},{f:"date",l:"Date",t:"date"},{f:"note",l:"Note (optional)",t:"text"}].map(({f,l,t})=>(
            <div key={f} style={{marginBottom:"16px"}}><label style={labelStyle}>{l}</label><input type={t} style={inputStyle} value={interestForm[f]} onChange={e=>setInterestForm({...interestForm,[f]:e.target.value})}/></div>
          ))}
      ...
      {showAddDividend&&(
        <Modal title="Log Dividend Payment" onClose={()=>setShowAddDividend(false)}>
          <div style={{marginBottom:"16px"}}><label style={labelStyle}>Asset</label><select style={{...inputStyle,cursor:"pointer"}} value={dividendForm.assetId} onChange={e=>setDividendForm({...dividendForm,assetId:e.target.value})}><option value="">— Select asset (optional)</option>{data.assets.map(a=><option key={a.id} value={a.id}>{a.ticker||a.name}</option>)}</select></div>
          {[{f:"amount",l:"Amount (€)",t:"number"},{f:"date",l:"Date",t:"date"},{f:"note",l:"Note (optional)",t:"text"}].map(({f,l,t})=>(
            <div key={f} style={{marginBottom:"16px"}}><label style={labelStyle}>{l}</label><input type={t} style={inputStyle} value={dividendForm[f]} onChange={e=>setDividendForm({...dividendForm,[f]:e.target.value})}/></div>
          ))}
      ...
      {showSnapshot&&(
        <Modal title="Save Portfolio Snapshot" onClose={()=>setShowSnapshot(false)}>
          <div style={{marginBottom:"20px"}}><label style={labelStyle}>Snapshot Date</label><input type="date" style={inputStyle} value={snapshotDate} onChange={e=>setSnapshotDate(e.target.value)}/></div>
          <div style={{background:"#1A1D1B",borderRadius:"4px",padding:"16px",marginBottom:"20px"}}>
            {[{l:"Securities",v:totalAssetsValue,color:"#C8A97E"},{l:"P2P Credit",v:totalP2PValue,color:"#7EC89B"},{l:"Real Estate",v:totalRealEstateValue,color:"#7EB5C8"},{l:"Commodities",v:totalCommoditiesValue,color:"#C8BC7E"},{l:"Total",v:totalValue,highlight:true}].map(r=>(
              <div key={r.l} style={{display:"flex",justifyContent:"space-between",marginBottom:"8px"}}>
                ...
                <span style={{color:r.highlight?"#C8A97E":(r.color||"#C0B8A8"),fontSize:r.highlight?"16px":"13px"}}>{formatCurrency(r.v)}</span>
              </div>
            ))}
          </div>
